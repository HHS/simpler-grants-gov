...
class EtlIssueModel(EtlDb):
    ...
    def sync_issue(self, issue_df: pandas.DataFrame, ghid_map: dict) -> (int, EtlChangeType):
    ...
    def _insert_dimensions(self, issue_df: pandas.DataFrame, ghid_map: dict) -> int:
    ...
    def _insert_facts(self, issue_id: int, issue_df: pandas.DataFrame, ghid_map: dict) -> (int, int):
        ...
        cursor = self.connection()
        insert_sql1 = "insert into gh_issue_history (issue_id, status, is_closed, points, d_effective) values (:issue_id, :status, :is_closed, :points, :effective) on conflict (issue_id, d_effective) do update set (status, is_closed, points, t_modified) = (:status, :is_closed, :points, current_timestamp) returning id" 
        ...
        insert_sql2 = "insert into gh_issue_sprint_map (issue_id, sprint_id, d_effective) values (:issue_id, :sprint_id, :effective) on conflict (issue_id, d_effective) do update set (sprint_id, t_modified) = (:sprint_id, current_timestamp) returning id"
    ...
    def _update_dimensions(self, issue_df: pandas.DataFrame, ghid_map: dict) -> (int, EtlChangeType):
