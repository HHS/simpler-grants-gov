WITH
  data_availability AS {{#160-data-availability-newest}},
  -- get project_id 
  project_data AS (
    SELECT
  ...
  -- get list of issue_ids in sprint 
  issue_id_list AS (
    SELECT DISTINCT
      (issue_id)
    FROM
      app.gh_issue_sprint_map AS ism,
      sprint_data
    WHERE
      ism.sprint_id = sprint_data.sprint_id
  ),
  -- get state of each issue on last day of sprint 
  issue_state AS (
    SELECT
      h.issue_id,
      h.points,
      ...
      END AS has_points
    FROM
      app.gh_issue_history AS h,
      sprint_data,
      data_availability
    WHERE
      h.sprint_id = sprint_data.sprint_id
      AND (
        (
          -- use last day of sprint as effective date IF it is in the past...
...
SELECT
  pointed.total AS pointed_total,
