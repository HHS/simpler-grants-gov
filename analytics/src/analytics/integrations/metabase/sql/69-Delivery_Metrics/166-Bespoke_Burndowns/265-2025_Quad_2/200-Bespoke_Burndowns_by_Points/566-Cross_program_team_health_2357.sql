WITH RECURSIVE -- 1. Define constant deliverable_ghid
 constants AS
  (SELECT 'HHS/simpler-grants-gov/issues/2357' AS deliverable_ghid), -- 2. Get reporting period dynamically
 reporting_period AS ({{#364-DEFAULT-reporting-period}}), -- 3. Resolve the selected deliverable using the constant deliverable_ghid
 selected_deliverable AS
  (SELECT id,
          ghid
   FROM gh_deliverable
   WHERE ghid =
       (SELECT deliverable_ghid
        FROM constants)), -- 4. Latest epic-to-deliverable mappings only (current mapping)
 latest_epic_mappings AS
  (SELECT DISTINCT ON (edm.epic_id) edm.epic_id,
   ...
   ORDER BY edm.epic_id,
            edm.d_effective DESC), -- 5. Epics currently mapped to the selected deliverable
 epics_in_deliverable AS
  (SELECT lem.epic_id,
          lem.epic_ghid,
          lem.deliverable_id
   FROM latest_epic_mappings lem
   JOIN selected_deliverable sd ON lem.deliverable_id = sd.id), -- 6. Recursively walk the issue tree from epics
 epic_issue_tree AS
  (SELECT e.deliverable_id,
   ...
   UNION ALL SELECT eit.deliverable_id,
                    ...
                    i.parent_issue_ghid
   FROM gh_issue i
   JOIN epic_issue_tree eit ON i.parent_issue_ghid = eit.issue_ghid), -- 7. Pre-filter issues that are direct children of the deliverable
 raw_direct_issues AS
  (SELECT sd.id AS deliverable_id,
   ...
   JOIN gh_issue i ON i.parent_issue_ghid = sd.ghid), -- 8. Filter out issues that are actually epics mapped elsewhere
 direct_issues AS
  (SELECT rdi.deliverable_id,
   ...
   WHERE NOT EXISTS
       (SELECT 1
        ...
        WHERE ep.ghid = rdi.issue_ghid)), -- 9. Combine both sources of issues
 combined_issues AS
  (SELECT *
   FROM epic_issue_tree
   UNION ALL SELECT *
   FROM direct_issues), -- 10. Calculate points opened in the given time period
 opened AS
  (SELECT h.d_effective AS issue_day,
   ...
   ORDER BY h.d_effective), -- 11. Calculate points closed in the given time period
 closed AS
  (SELECT h.d_effective AS issue_day,
   ...
   GROUP BY h.d_effective
   ORDER BY h.d_effective), -- 12. Aggregate points opened and closed by day
 totals AS
...
SELECT
  (SELECT deliverable_title
   FROM
     (SELECT title AS deliverable_title
      FROM gh_deliverable
      WHERE ghid =
          (SELECT deliverable_ghid
           FROM constants)) AS t) AS deliverable_title,

  (SELECT CONCAT('https://github.com/', deliverable_ghid)
