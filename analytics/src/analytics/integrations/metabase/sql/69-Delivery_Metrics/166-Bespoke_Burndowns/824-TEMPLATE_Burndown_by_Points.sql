WITH RECURSIVE -- constants
 constants AS
  (SELECT 'HHS/simpler-grants-gov/issues/XXXX' AS deliverable_ghid -- REPLACE XXXX with actual deliverable ghid
), -- Get reporting period dynamically
 reporting_period AS ({{#364-DEFAULT-reporting-period}}), -- Resolve the selected deliverable using the constant deliverable_ghid
  ...
  (SELECT id,
   ...
   WHERE ghid =
       (SELECT deliverable_ghid
        FROM constants)), -- Get latest epic-to-deliverable mappings only
 ...
 epics_in_deliverable AS
  (SELECT lem.epic_id,
   ...
   JOIN selected_deliverable sd ON lem.deliverable_id = sd.id), -- Recursively walk the issue tree from epics
 epic_issue_tree AS
  (SELECT e.deliverable_id,
   ...
   UNION ALL SELECT eit.deliverable_id,
                    ...
                    i.parent_issue_ghid
   FROM gh_issue i
   JOIN epic_issue_tree eit ON i.parent_issue_ghid = eit.issue_ghid), -- Filter issues that are direct children of the deliverable
 raw_direct_issues AS
  (SELECT sd.id AS deliverable_id,
   ...
   JOIN gh_issue i ON i.parent_issue_ghid = sd.ghid), -- Filter issues that are actually epics mapped elsewhere
 direct_issues AS
