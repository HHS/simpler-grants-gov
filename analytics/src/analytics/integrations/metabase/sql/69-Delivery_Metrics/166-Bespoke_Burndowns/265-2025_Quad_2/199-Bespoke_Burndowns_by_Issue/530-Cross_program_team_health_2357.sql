  ...
  (SELECT id,
   ...
   WHERE ghid =
       (SELECT deliverable_ghid
        FROM constants)), -- 4. Latest epic-to-deliverable mappings only
  ...
  (SELECT rdi.deliverable_id,
   ...
   WHERE NOT EXISTS
       (SELECT 1
        ...
        WHERE ep.ghid = rdi.issue_ghid)), -- 9. Combine both sources
 combined_issues AS
  (SELECT *
   FROM epic_issue_tree
   UNION ALL SELECT *
   FROM direct_issues), -- 10. Calculate issues opened in the given time period
 opened AS
  (SELECT h.d_effective AS issue_day,
   ...
   ORDER BY h.d_effective), -- 11. Calculate issues closed in the given time period
 closed AS
  (SELECT h.d_effective AS issue_day,
   ...
   GROUP BY h.d_effective
   ORDER BY h.d_effective), -- 12. Aggregate issues opened and closed by day
 totals AS
