   ...
   WHERE quad_id =
     ...
     AND ranked_order =
       (SELECT deliverable_ranked_order
        FROM constants)), -- Precomputed daily opened/closed points for the selected deliverable,
 -- filtered to the reporting period
 totals AS
  (SELECT b.issue_day,
          b.total_points_opened AS total_opened,
          b.total_points_closed AS total_closed,
          b.total_points_opened - b.total_points_closed AS total_remaining
   ...
   WHERE b.issue_day BETWEEN reporting_period.start_date AND reporting_period.end_date)
