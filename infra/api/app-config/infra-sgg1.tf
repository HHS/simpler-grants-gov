# api service for the infra-sgg1 environment (AWS account 061664787759, network_name "infra-sgg1").
#
# infra-sgg1 is the "dev" account's copy of the shared account's sgg1
# environment. Config values below mirror infra/api/app-config/sgg1.tf 1:1,
...
module "infra_sgg1_config" {
  source         = "./env-config"
  project_name   = local.project_name
  app_name       = local.app_name
  default_region = module.project_config.default_region
  environment    = "infra-sgg1"
  network_name   = "infra-sgg1"

  # Reuses sgg1's hostnames. domain_name cannot be null -- see the comment in
  ...
  database_newrelic_entity_guid = "" # Populate once the New Relic entity for the infra-sgg1 RDS cluster exists
  database_deletion_protection  = false

  has_incident_management_service = local.has_incident_management_service
  enable_identity_provider        = local.enable_identity_provider
  enable_notifications            = false # Enable once an SES domain identity exists for infra-sgg1

  service_newrelic_entity_guid      = "" # Populate once the New Relic entity for the infra-sgg1 primary ALB exists
  service_newrelic_mtls_entity_guid = "" # Populate once the New Relic entity for the infra-sgg1 mTLS ALB exists
  api_host_newrelic_entity_guid     = "" # Populate once the New Relic entity for the infra-sgg1 ECS service host exists
  ...
  service_override_extra_environment_variables = {
    ...
    # PDF Generation
    FRONTEND_URL             = "https://sgg1.teams.simpler.grants.gov" # sgg1's frontend; repoint once infra-sgg1 has DNS
