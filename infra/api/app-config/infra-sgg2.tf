# api service for the infra-sgg2 environment (AWS account 061664787759, network_name "infra-sgg2").
#
# infra-sgg2 is a team environment in the "dev" account. It replaced infra-grantee1, which
...
module "infra_sgg2_config" {
  ...
  database_newrelic_entity_guid = "" # Populate once the New Relic entity for the infra-sgg2 RDS cluster exists
  database_deletion_protection  = false

  has_incident_management_service = local.has_incident_management_service
  enable_identity_provider        = local.enable_identity_provider
  enable_notifications            = false # Enable once an SES domain identity exists for infra-sgg2

  service_newrelic_entity_guid      = "" # Populate once the New Relic entity for the infra-sgg2 primary ALB exists
  service_newrelic_mtls_entity_guid = "" # Populate once the New Relic entity for the infra-sgg2 mTLS ALB exists
  api_host_newrelic_entity_guid     = "" # Populate once the New Relic entity for the infra-sgg2 ECS service host exists
  ...
  service_override_extra_environment_variables = {
    ...
    # PDF Generation
    FRONTEND_URL             = "https://sgg2.teams.simpler.grants.gov"
