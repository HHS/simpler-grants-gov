...
module "search" {
  ...
  sso_admin_role_arn = local.search_config.sso_admin_role_name != null ? "arn:aws:iam::${data.aws_caller_identity.current.account_id}:role/aws-reserved/sso.amazonaws.com/${local.search_config.sso_admin_role_name}" : null
}
