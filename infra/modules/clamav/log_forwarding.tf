#--------------------------------------------
# Forward ClamAV Lambda CloudWatch logs to New Relic
#--------------------------------------------

locals {
  nr_log_forwarder_name = "${var.name}-nr-log-forwarder"
}
...
resource "aws_lambda_function" "nr_log_forwarder" {
  ...
  description   = "Forwards ClamAV scanner and freshclam CloudWatch logs to New Relic"
  runtime       = "python3.12"
...
resource "aws_cloudwatch_log_group" "nr_log_forwarder" {
  # checkov:skip=CKV_AWS_158:Forwarder logs hold only delivery diagnostics — AWS-managed encryption is sufficient, consistent with the other ClamAV Lambda log groups
  # checkov:skip=CKV_AWS_338:Forwarding Lambda logs don't need long retention — the actual ClamAV logs live in New Relic
...
resource "aws_lambda_permission" "allow_cloudwatch_scanner" {
  statement_id  = "AllowCloudWatchScanner"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.nr_log_forwarder.function_name
  principal     = "logs.amazonaws.com"
  source_arn    = "${aws_cloudwatch_log_group.scanner.arn}:*"
}

resource "aws_cloudwatch_log_subscription_filter" "scanner_to_newrelic" {
  name            = "${local.scanner_name}-to-newrelic"
  log_group_name  = aws_cloudwatch_log_group.scanner.name
  filter_pattern  = ""
  destination_arn = aws_lambda_function.nr_log_forwarder.arn

  depends_on = [aws_lambda_permission.allow_cloudwatch_scanner]
}

resource "aws_lambda_permission" "allow_cloudwatch_freshclam" {
  statement_id  = "AllowCloudWatchFreshclam"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.nr_log_forwarder.function_name
  principal     = "logs.amazonaws.com"
  source_arn    = "${aws_cloudwatch_log_group.freshclam.arn}:*"
}

resource "aws_cloudwatch_log_subscription_filter" "freshclam_to_newrelic" {
  name            = "${local.freshclam_name}-to-newrelic"
  log_group_name  = aws_cloudwatch_log_group.freshclam.name
  filter_pattern  = ""
  destination_arn = aws_lambda_function.nr_log_forwarder.arn

  depends_on = [aws_lambda_permission.allow_cloudwatch_freshclam]
}
