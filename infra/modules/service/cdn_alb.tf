resource "aws_cloudfront_origin_access_identity" "alb_cdn" {
  count   = var.enable_alb_cdn ? 1 : 0
  comment = "Origin Access Identity for CloudFront to access S3 bucket"
}

resource "aws_cloudfront_cache_policy" "alb_cdn" {
  count = var.enable_alb_cdn ? 1 : 0
...
resource "aws_cloudfront_distribution" "alb_cdn" {
  ...
  enabled = var.enable_alb_cdn ? true : false
  aliases = var.domain == null ? null : [var.domain]

  origin {
    domain_name = aws_lb.alb[0].dns_name
    origin_id   = local.cdn_default_origin_id
    custom_origin_config {
      ...
      origin_ssl_protocols = local.cdn_ssl_protocols
    }
  ...
  logging_config {
    ...
    bucket          = aws_s3_bucket.alb_cdn[0].bucket_domain_name
  }

  default_cache_behavior {
    allowed_methods        = ["DELETE", "GET", "HEAD", "OPTIONS", "PATCH", "POST", "PUT"]
    cached_methods         = ["GET", "HEAD", "OPTIONS"]
    target_origin_id       = local.cdn_default_origin_id
  ...
  viewer_certificate {
    ...
    minimum_protocol_version       = local.cdn_minimum_protocol_version
    ssl_support_method             = "sni-only"
  }

  depends_on = [
    aws_s3_bucket_public_access_block.alb_cdn[0],
    aws_s3_bucket_policy.alb_cdn[0],
    aws_s3_bucket.alb_cdn[0],
  ]
...
resource "aws_s3_bucket" "alb_cdn" {
  count = var.enable_alb_cdn ? 1 : 0
...
resource "aws_s3_bucket_ownership_controls" "alb_cdn" {
  count = var.enable_alb_cdn ? 1 : 0

  bucket = aws_s3_bucket.alb_cdn[0].id
  rule {
...
resource "aws_s3_bucket_acl" "alb_cdn" {
  count = var.enable_alb_cdn ? 1 : 0
  ...
  depends_on = [aws_s3_bucket_ownership_controls.alb_cdn[0]]
}

resource "aws_s3_bucket_public_access_block" "alb_cdn" {
  count = var.enable_alb_cdn ? 1 : 0
...
data "aws_iam_policy_document" "alb_cdn" {
  count = var.enable_alb_cdn ? 1 : 0
  ...
  statement {
    ...
    resources = [
      "${aws_s3_bucket.alb_cdn[0].arn}/*",
    ]

    principals {
      type        = "AWS"
      identifiers = [aws_cloudfront_origin_access_identity.alb_cdn[0].iam_arn]
    }
  }
}

resource "aws_s3_bucket_policy" "alb_cdn" {
  count = var.enable_alb_cdn ? 1 : 0

  bucket = aws_s3_bucket.alb_cdn[0].id
  policy = data.aws_iam_policy_document.alb_cdn[0].json
}

resource "aws_s3_bucket_lifecycle_configuration" "alb_cdn" {
  count = var.enable_alb_cdn ? 1 : 0
