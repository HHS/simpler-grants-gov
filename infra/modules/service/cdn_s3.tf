resource "aws_cloudfront_origin_access_identity" "s3_cdn" {
  count   = var.enable_s3_cdn ? 1 : 0
  comment = "Origin Access Identity for CloudFront to access S3 bucket"
}

resource "aws_cloudfront_cache_policy" "s3_cdn" {
  count = var.enable_s3_cdn ? 1 : 0
...
resource "aws_cloudfront_distribution" "s3_cdn" {
  ...
  enabled = var.enable_s3_cdn ? true : false
  aliases = var.domain == null ? null : [var.domain]

  origin {
    domain_name = aws_s3_bucket.s3_buckets[var.s3_cdn_bucket_name].bucket_regional_domain_name
  ...
  logging_config {
    ...
    bucket          = aws_s3_bucket.s3_cdn[0].bucket_domain_name
  }
  ...
  depends_on = [
    aws_s3_bucket_public_access_block.s3_cdn[0],
    aws_s3_bucket_policy.s3_cdn[0],
    aws_s3_bucket.s3_cdn[0],
  ]
...
resource "aws_s3_bucket_ownership_controls" "s3_cdn" {
  count = var.enable_s3_cdn ? 1 : 0

  bucket = aws_s3_bucket.s3_cdn[0].id
  rule {
...
resource "aws_s3_bucket_acl" "s3_cdn" {
  count = var.enable_s3_cdn ? 1 : 0
  ...
  depends_on = [aws_s3_bucket_ownership_controls.s3_cdn[0]]
}

resource "aws_s3_bucket_public_access_block" "s3_cdn" {
  count = var.enable_s3_cdn ? 1 : 0
...
data "aws_iam_policy_document" "s3_cdn" {
  ...
  statement {
    ...
    resources = [
      "${aws_s3_bucket.s3_cdn[0].arn}/*",
    ]

    principals {
      type        = "AWS"
      identifiers = [aws_cloudfront_origin_access_identity.s3_cdn[0].iam_arn]
    }
  }
}

resource "aws_s3_bucket_policy" "s3_cdn" {
  count = var.enable_s3_cdn ? 1 : 0

  bucket = aws_s3_bucket.s3_cdn[0].id
  policy = data.aws_iam_policy_document.s3_cdn[0].json
}

resource "aws_s3_bucket_lifecycle_configuration" "s3_cdn" {
  count = var.enable_s3_cdn ? 1 : 0
