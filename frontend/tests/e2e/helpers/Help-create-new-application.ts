...
export async function Help_createNewApplication(
  ...
  if (isMobile && isChrome) {
    ...
    if (elementHandle) {
      ...
      await safeLog(testInfo, "⚠️ Used JS click fallback for mobile Chrome on start application modal");
    } else {
      await safeLog(testInfo, "❌ Could not find button element for JS click fallback.", "error");
    }
  } else {
  ...
  if (isMobile && isChrome) {
    await Promise.race([
      (async () => {
        await mobileChromeAction(page, page.getByTestId("application-start-save"), "click", undefined, undefined, testInfo);
      })(),
      new Promise((_, reject) => setTimeout(() => reject(new Error("Timeout exceeded")), 10000))
    ]);
  } else {
    await safeHelp_clickButton(
      testInfo,
...
async function mobileChromeAction(
  ...
  if (isMobile && isChrome) {
    ...
    if (!elementHandle) throw new Error("Element not found for mobile Chrome fallback.");
    if (action === "click") {
      await locator.scrollIntoViewIfNeeded();
      await elementHandle.evaluate((el: Element) => (el as HTMLElement).click());
      if (testInfo) await safeLog(testInfo, "⚠️ Used JS click fallback for mobile Chrome");
    } else if (action === "fill" && value !== undefined) {
      await locator.scrollIntoViewIfNeeded();
      await elementHandle.evaluate((el: Element, v: string) => ((el as HTMLInputElement).value = v), value);
      if (testInfo) await safeLog(testInfo, "⚠️ Used JS fill fallback for mobile Chrome");
    } else if (action === "expect" && value !== undefined) {
      const text: string = await elementHandle.evaluate((el: Element) => (el as HTMLElement).textContent || "");
      if (!text.includes(value)) {
        throw new Error(`Expected text "${value}" not found in element (mobile Chrome fallback).`);
      }
      if (testInfo) await safeLog(testInfo, "⚠️ Used JS expect fallback for mobile Chrome");
    }
  } else {
    if (action === "click") {
      await locator.click();
...
// Usage example:
// await mobileChromeAction(page, page.getByTestId("open-start-application-modal-button"), "click", undefined, testInfo);
// await mobileChromeAction(page, page.getByTestId("textInput"), "fill", "my value", testInfo);
// await mobileChromeAction(page, page.getByTestId("some-label"), "expect", "Expected Text", testInfo);
