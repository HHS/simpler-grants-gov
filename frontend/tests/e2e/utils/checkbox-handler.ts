...
export const checkboxHandler: FieldHandler = async (testInfo, page, field, data) => {
  if (shouldActivateField(data)) {
    ...
    try {
      ...
      if ((await nestedCheckbox.count()) === 0) {
        throw new Error(`Checkbox field ${field.field} is not checkable; map to the checkbox input testId`);
      }
