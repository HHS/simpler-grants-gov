...
export type PageFillFieldDefinitions = {
  [fieldIdentifier: string]: PageFillFieldDefinition;
};
  ...
  try {
    ...
    if (field.type === "dropdown") {
      ...
      if (field.label) {
        ...
        try {
          await locator.selectOption({ value: data });
        } catch {
          await locator.selectOption({ label: data });
        }
      ...
      throw new Error(
        `Dropdown field ${fieldIdentifier} is missing selector, label, or testId`,
      );
      ...
      field.type === "text" &&
      (field.testId || field.selector || field.label) &&
      typeof data === "string"
    ) {
