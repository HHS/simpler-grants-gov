...
for (const { testName, orgLabel } of applicantScenarios) {
  test(
    ...
    async (
      ...
      // Federal Funding Share = Table 2 row 0 (line 16c) × 90% = 6,300 × 0.9 = 5,670
      const expectedFederalFundingShare = 5_670;

      // Table 2 (Federal Funding) validation:
      // Row 0: Total project costs (read-only) maps to line 16c (allowable total) = $6,300
