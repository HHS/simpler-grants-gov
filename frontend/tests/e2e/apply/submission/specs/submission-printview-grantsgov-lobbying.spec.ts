/**
 * @feature Apply - Happy Path - Grants.gov Lobbying Form Submission and Print View Workflow
 * @scenario Complete the Grants.gov Lobbying Form Submission and Print View workflow for an <user type> user
 */
...
const OPPORTUNITY_NUMBER = "E2E-GGLOB-ORG-IND-01";
const opportunityConfig = loadOpportunityConfig(OPPORTUNITY_NUMBER);

const applicantScenarios = [
  {
    testName: `Complete the Grants.gov Lobbying Form Submission and Print View workflow for an Organization user`,
    orgLabel: testOrgLabel,
  },
  {
    testName: `Complete the Grants.gov Lobbying Form Submission and Print View workflow for an Individual user`,
    orgLabel: undefined,
  },
] as const;
...
for (const { testName, orgLabel } of applicantScenarios) {
  test(
    ...
    async (
      ...
      // And the user launches the URL for an opportunity with an open Grants.gov Lobbying Form competition
      ...
      // This opportunity contains a single form (Grants.gov Lobbying Form).
      ...
      // --- Post-Population Field Validation (Signature and Submission Date) ---
