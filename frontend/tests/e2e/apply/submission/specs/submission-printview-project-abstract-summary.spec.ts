...
const { APPLY, SMOKE, GRANTEE, APPLY_FORMS, CORE_REGRESSION } = VALID_TAGS;
...
for (const { testName, orgLabel } of applicantScenarios) {
  test(
    ...
    async (
      ...
      // Iterate each form on the opportunity, build unique test data, then fill it.
      // `baseSuffix + index` guarantees unique values across concurrent runs.
      for (const [index, form] of opportunityConfig.forms.entries()) {
        const testData = buildHappyPathTestData(
          form.formKey, // which form's builder to use
          baseSuffix + index, // unique suffix per form instance
          form.formConfig,
        );

        // Fill the form using generated testData (last arg false = do not submit/navigate)
        ...
        // Verify form status on application landing page
        ...
        // Save an in-memory snapshot of this form for later print-view assertions:
        // includes `testData`, the `printUrl`, expected prepopulated values
        filledForms.push({
