...
test("SF-424B error validation - required fields and inline errors", async ({
  ...
  if ((await sf424bLink.count()) > 0) {
    ...
    await expect(alertList).toContainText('Title is required');
    await expect(alertList).toContainText('Applicant Organization is required');

    // Scroll down to check for inline errors
    await page.evaluate(() => window.scrollTo(0, document.body.scrollHeight));
    await expect(page.locator('#error-for-title')).toBeVisible();
    await expect(page.locator('#error-for-title')).toContainText('Title is required');
