 ...
 * @scenario Application form completion failure path - project-abstract
 */
...
test(
  "Project Abstract - error validation on empty save",
  ...
  async (
    ...
    // And the Application landing page loads with the "Project Abstract" form link visible
    // And the user clicks on a form link
    // Then the form opens
    const opened = await openForm(page, PROJECT_ABSTRACT_FORM_MATCHER);
    if (!opened) {
      throw new Error(
    ...
    // Then under the "Project Abstract" form the status shows "Some issues found. Check your entries."
