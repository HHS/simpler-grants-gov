...
Rule: When multiple pages of results exist

  Scenario: Pagination controls are visible when multiple pages exist
    Given the search returns more than one page of results
  ...
  Scenario: User navigates using page number
    Given the search returns at least 3 pages of results
    And I am on page 1
...
Rule: When only one page of results exists
