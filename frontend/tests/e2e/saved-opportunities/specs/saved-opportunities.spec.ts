...
test.describe("Saved Opportunities", () => {
  ...
  test(
    ...
    async ({ page, context }, { project }) => {
      ...
      // And the funding opportunity number on the detail page should match the card
      // "Opportunity Number: TEST-APPLY-ORG-IND-ON01" on the saved card corresponds to
      // "Funding opportunity number: TEST-APPLY-ORG-IND-ON01" on the detail page,
      // confirming navigation landed on the correct record.
      await expect(page.getByTestId("opportunity-intro-content")).toContainText(
        SAVED_OPPORTUNITY.opportunityNumber,
      );
