...
describe('Your API Route', () => {
  ...
  it('should return 405 if method is not POST', async () => {
    await handler({ ...mockRequest, method: 'GET' } as NextApiRequest, mockResponse);
    expect(mockResponse.status).toHaveBeenCalledWith(405);
    expect(mockResponse.json).toHaveBeenCalledWith({ error: 'Method not allowed' });
  });

  it('should return 400 if name or email is missing', async () => {
    await handler({ ...mockRequest, body: JSON.stringify({}) } as NextApiRequest, mockResponse);
  ...
  it('should return 500 if there is an error during subscription', async () => {
    ...
    expect(console.error).toHaveBeenCalledWith("Error subscribing user:", "Some fetch error")
