...
>({ id, value: rawValue = [], rawErrors }: UswdsWidgetProps<T, S, F>) {
  ...
  //  B) value = { activity_line_items: [...], total_budget_categories: {...} }
  ...
  const totals: AnyObj | undefined = Array.isArray(rawValue)
    ? undefined
    : ((rawValue as AnyObj).total_budget_categories as AnyObj) || undefined;
  ...
  // Section B rows (A–K)
  const ROW_I_KEY = "total_direct_charge_amount"; // row i
  const ROW_K_KEY = "total_amount"; // row k
  ...
  type RowDef = { key: string; label: string; letter: string; note?: string };
  ...
  const totalInput = ({ fieldKey }: { fieldKey: string }) => {
      ...
      >
        ...
        <tbody>
          <tr className="bg-base-lightest">
            <th scope="row" className="padding-05 text-bold border-bottom-0 verticle-align-bottom">
            ...
            <tr key={r.key} className="sf424a__row">
              ...
              {COLUMNS.map((columnIndex) => (
                <td
                  ...
                  className={`border-bottom-0 border-top-0 padding-05 verticle-align-top sf424a__cell ${r.key !== ROW_I_KEY && r.key !== ROW_K_KEY ? "padding-top-5" : null}`}
                  height={'inherit'}
                >
                  <div className="display-flex flex-column sf424a__cell-content">
