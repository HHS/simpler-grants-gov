...
>({
  ...
  // Local selection for instant UI feedback
  const [picked, setPicked] = useState<"" | "true" | "false">(
    () => toBoolString(value),
  );

  // Only sync from parent when it actually supplies a boolean.
  // If parent transiently passes `undefined` during a save/remount, keep local pick.
  useEffect(() => {
  ...
  const handleBlur = useCallback(
    ({ target }: FocusEvent<HTMLInputElement>) => onBlur(id, target?.value === "true"),
    [onBlur, id],
  );

  const handleFocus = useCallback(
    ({ target }: FocusEvent<HTMLInputElement>) => onFocus(id, target?.value === "true"),
    [onFocus, id],
  );
  ...
  const handlePick = useCallback(
    (nextStr: "true" | "false") => {
      ...
      if (typeof onChange === "function") {
        onChange(nextStr === "true"); // push boolean up
      }
    },
    [onChange],
  );
      ...
      {Array.isArray(enumOptions) &&
        enumOptions.map((option, i) => {
          ...
          const onChangeRadio = (e: ChangeEvent<HTMLInputElement>) =>
            handlePick(e.target.value as "true" | "false");
