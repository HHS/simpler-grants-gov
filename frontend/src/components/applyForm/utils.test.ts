...
describe("processFormSchema", () => {
  ...
  it("calls extricateConditionalValidationRules with formSchema.properties", async () => {
  ...
  it("defaults to empty properties when formSchema.properties is undefined", async () => {
  ...
  it("calls mergeAllOf with the properties returned from extricateConditionalValidationRules", async () => {
    ...
    mockExtricateConditionalValidationRules.mockReturnValue({
      propertiesWithoutComplexConditionals:
        propertiesWithoutComplexConditionals as unknown as RJSFSchema,
      conditionalValidationRules: { path: [{ rule: "something " }] },
    });

    await processFormSchema({ properties: { ignored: { type: "string" } } });
  ...
  it("returns the expected combination of original schema and merged properties", async () => {
  ...
  it("rethrows if a processor throws", async () => {
    mockExtricateConditionalValidationRules.mockImplementation(() => {
      throw new Error("boom");
    });

    await expect(processFormSchema({ properties: {} })).rejects.toThrow("boom");
  });
});
