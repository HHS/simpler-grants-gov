...
@workflow_blueprint.cli.command("create-workflow-diagrams")
def create_workflow_diagrams() -> None:
    ...
    diagram_folder = parent_folder / "state_machine" / "diagrams"
    ...
    for _, state_machine_cls in WorkflowRegistry._workflow_registry.values():
        logger.info(f"Creating workflow diagram for {state_machine_cls.__name__}")
