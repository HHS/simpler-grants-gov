...
class PostgresDBClient(DBClient):
    ...
    def check_db_connection(self) -> None:
        with self.get_connection() as conn:
            # dbapi_connection is Optional on the SQLAlchemy protocol, but it is always
            # populated for a connection we just opened.
            conn_info = conn.connection.dbapi_connection.info  # type: ignore[union-attr]
