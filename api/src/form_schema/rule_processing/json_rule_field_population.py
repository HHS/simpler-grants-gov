...
def get_opportunity_number(context: JsonRuleContext) -> str:
    return context.opportunity.opportunity_number

def get_opportunity_title(context: JsonRuleContext) -> str:
    return context.opportunity.opportunity_title

def get_agency_name(context: JsonRuleContext) -> str:
...
def get_signature(context: JsonRuleContext) -> str:
...
def handle_field_population(context: JsonRuleContext, rule: dict, path: list[str], mapper: dict[str, population_func]) -> None:
    rule_code: str | None = rule.get("rule", None)

    log_extra = context.get_log_context() | {"field_population_rule": rule_code, "path": ".".join(path)}
