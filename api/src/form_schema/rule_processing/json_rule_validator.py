...
from src.form_schema.rule_processing.json_rule_util import get_nested_value, build_path_str
...
def validate_attachments(context: JsonRuleContext, rule: dict, path: list[str]) -> None:
    ...
    """
    application_attachment_ids = [str(application_attachment.application_attachment_id) for application_attachment in context.application_form.application.application_attachments]
    ...
    if isinstance(value, str):
        if value not in application_attachment_ids:
            context.validation_issues.append(ValidationErrorDetail(
