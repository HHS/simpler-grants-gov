...
from src.db.models.opportunity_models import OpportunityChangeAudit, Opportunity, OpportunityVersion
from src.db.models.task_models import JobLog
from src.services.opportunities_v1.opportunity_version import SCHEMA, save_opportunity_version
...
class StoreOpportunityVersionTask(Task):
    ...
    def run_task(self) -> None:
        ...
        latest_job = self.db_session.scalars(
            select(JobLog).where(JobLog.job_type==self.cls_name()).where(or_(JobLog.job_status==JobStatus.COMPLETED,JobLog.job_status==JobStatus.FAILED)).order_by(JobLog.created_at.desc()) # get the latest # add job_status
        ).first()
        # Get opportunity ids that were updated after the latest job run
        latest_time = latest_job.created_at  if latest_job else get_now_us_eastern_datetime() - timedelta(hours=24)  #tbc
        ...
        for opp in updated_opportunities:
            ...
            if latest_versioned_opp:
                diffs = diff_nested_dicts(opportunity_v1, latest_versioned_opp)
                if diffs:
                    save_opportunity_version(self.db_session, opp)
