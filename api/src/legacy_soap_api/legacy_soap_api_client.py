...
class SimplerGrantorsS2SClient(BaseSOAPClient):
    ...
    def get_simpler_soap_response(self, proxy_response: SOAPResponse) -> tuple:
        ...
        simpler_response_xml = build_mtom_xml_from_dict(
            self.operation_config.namespaces, simpler_response_soap_dict
        )
        boundary_uuid = f"{uuid.uuid4()}"
        update_headers = {
            "MIME-Version": "1.0",
        ...
        boundary = b"--uuid:" + boundary_uuid.encode("utf-8") + b"\n"
        ...
        mime_message += simpler_response_xml + b"\n"
        mime_message += boundary
        use_simpler_response = False
        return (
