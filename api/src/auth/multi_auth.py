...
from .api_gateway_key_auth import api_gateway_key_auth
...
class AuthType(StrEnum):
    ...
    API_GATEWAY_KEY_AUTH = "api_gateway_key_auth"
...
class MultiHttpTokenAuth(MultiAuth):
    ...
    def get_user(self) -> MultiAuthUser:
        ...
        if isinstance(current_user, ApiKeyUser):
            ...
            return MultiAuthUser(current_user, AuthType.API_GATEWAY_KEY_AUTH)
...
#
# Note that the order matters - api_gateway_key_auth will take precedence if both headers are present
api_key_multi_auth = MultiHttpTokenAuth(api_gateway_key_auth, api_key_auth)
...
api_key_multi_auth_security_schemes = _get_security_requirement(
    [
        api_gateway_key_auth.security_scheme_name,
