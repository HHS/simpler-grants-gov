...
class XMLGenerationService:
    ...
    def _generate_xml_string(
        ...
        if namespace_fields or default_namespace:
            return self._generate_xml_with_namespaces(
                data, root_element_name, namespace_config, namespace_fields, pretty_print, transform_config
            )
        else:
    ...
    def _generate_xml_with_namespaces(
        ...
        xsd_url = xml_config.get("xsd_url", "https://apply07.grants.gov/apply/forms/schemas/SF424_4_0-V4.0.xsd")
    ...
    def _get_xsd_element_order(self, xsd_url: str, root_element: str = "SF424_4_0") -> list[str]:
        ...
        try:
            ...
            root_element_def = xsd_root.xpath(f"//xs:element[@name='{root_element}']", namespaces=xs_ns)
            
            if not root_element_def:
                raise ValueError(f"Root element '{root_element}' not found in XSD schema at {xsd_url}")
            ...
            if not elements:
                raise ValueError(f"No elements found in XSD schema at {xsd_url} for root element '{root_element}'")
    ...
    def _get_complex_type_element_order(
        self, xsd_url: str, complex_type_name: str, import_namespaces: tuple[str, ...] = None
    ) -> list[str]:
        ...
        try:
            ...
            complex_type = xsd_root.xpath(f"//xs:complexType[@name='{complex_type_name}']", namespaces=xs_ns)
            ...
            if not complex_type and import_namespaces:
                for namespace_uri in import_namespaces:
                    imports = xsd_root.xpath(f"//xs:import[@namespace='{namespace_uri}']", namespaces=xs_ns)
                    for import_elem in imports:
                        ...
                        if import_url:
                            try:
                                ...
                                complex_type = import_xsd.xpath(f"//xs:complexType[@name='{complex_type_name}']", namespaces=xs_ns)
                                if complex_type:
                                    logger.info(f"Found {complex_type_name} in imported schema: {import_url}")
                                    break
                            except Exception as e:
                                logger.warning(f"Failed to fetch imported XSD from {import_url}: {e}")
                    if complex_type:
                        break
            
            if not complex_type:
                raise ValueError(f"ComplexType '{complex_type_name}' not found in XSD schema at {xsd_url}")
            
            # Extract elements from the complexType
            elements = self._extract_elements_from_complex_type(complex_type[0], xs_ns)
            
            if not elements:
                raise ValueError(f"No elements found in complexType '{complex_type_name}' at {xsd_url}")
                
            logger.info(f"Extracted {len(elements)} elements from complexType '{complex_type_name}'")
            return elements
                
        except Exception as e:
            logger.error(f"Failed to extract elements from complexType '{complex_type_name}' in XSD {xsd_url}: {e}")
            raise ValueError(f"Could not parse complexType '{complex_type_name}' from XSD schema at {xsd_url}: {e}") from e
    ...
    def get_element_order_for_type(self, xsd_url: str, type_name: str, search_imports: bool = True) -> list[str]:
        ...
        ) if search_imports else None
        
        return self._get_complex_type_element_order(
            xsd_url=xsd_url,
            complex_type_name=type_name,
            import_namespaces=common_imports
        )

    def _add_lxml_element_to_parent(
        self, parent: Any, field_name: str, value: Any, nsmap: dict, namespace_fields: dict, xsd_url: str = None
    ) -> None:
