...
class XMLGenerationService:
    ...
    def generate_xml(
        self, db_session: "db.Session", request: XMLGenerationRequest
        ...
        try:
            ...
            if not application_data:
                return XMLGenerationResponse(
                    success=False, error_message="No application response data found"
                )
            ...
            self.logger.info(
                f"XML generation successful: {len(transformed_data)} fields transformed from {len(application_data)} input fields for {request.form_name}"
            )
    ...
    def _get_application_form(
        self, db_session: "db.Session", application_form_id: UUID
        ...
        return (
            db_session.query(ApplicationForm)
            .filter(ApplicationForm.id == application_form_id)  # type: ignore[attr-defined]
            .first()
        )
