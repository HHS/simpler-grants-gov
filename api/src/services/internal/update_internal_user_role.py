...
from src.db.models.user_models import LinkExternalUser, Role, InternalUserRole, LinkRoleRoleType

logger = logging.getLogger(__name__)

def update_internal_user_role(db_session: db.Session, internal_role_id: str, user_email: str) -> None:
    # Query the role table and verify it is an internal role
    role = db_session.query(Role).join(
        LinkRoleRoleType, Role.role_id == LinkRoleRoleType.role_id
    ).filter(
        Role.role_id == internal_role_id,
        LinkRoleRoleType.role_type == RoleType.INTERNAL
    ).first()
    ...
    external_user = db_session.query(LinkExternalUser).filter(
        LinkExternalUser.email == user_email
    ).one_or_none()
    ...
    existing_assignment = db_session.query(InternalUserRole).filter_by(
        user_id=target_user_id,
        role_id=internal_role_id
    ).first()
