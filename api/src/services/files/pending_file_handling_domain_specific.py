...
def fetch_and_validate_scan_complete_file(
    ...
    """
    fetch and validate a pending file record with scan complete status, and authorization checks.

    This function:
    - Fetch file record by ID (raises 404 if not found)
    - Verifies the user attached to the record matches the calling user (raises 403 if not)
    - Verifies the file is marked as complete (raises 422 if not)

    Args:
        db_session: Database session
        pending_file_id: ID of the file being fetched
        user: to verify the permission

    Returns:
        The validated file record

    Raises:
        404: If the pending file is not found
    ...
    if pending_file is None:
        logger.warning(
            ...
            extra={"pending_file_id": pending_file_id, "user_id": user.user_id},
        )
