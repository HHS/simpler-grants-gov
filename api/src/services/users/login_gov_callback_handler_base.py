    ...
    def handle_token(self, login_gov_data: LoginGovDataContainer) -> LoginGovCallbackResponse:
        ...
        while tries < limit:
            ...
            response = client.get_token(
                OauthTokenRequest(
                    # handle_callback_request 422s on a missing code, so it is set by here.
                    code=login_gov_data.code,  # type: ignore[arg-type]
                    client_assertion=get_login_gov_client_assertion(),
                )
            )
