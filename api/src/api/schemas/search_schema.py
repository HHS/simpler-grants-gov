...
class BaseSearchSchemaBuilder:
    ...
    def build(self) -> Schema:
        # from_dict is typed against plain marshmallow Schema/Field rather than our
        # extension subclasses, so the class it hands back doesn't line up statically.
        return BaseSearchSchema.from_dict(  # type: ignore[return-value]
            self.schema_fields,  # type: ignore[arg-type]
            name=self.schema_class_name,
        )
