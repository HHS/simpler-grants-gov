...
# These models exercise the generic DB, lookup, and auth base classes without
# coupling those tests to the real application tables. They have no migrations -
# they attach to the same metadata as the real models, so the test DB fixture
# creates them alongside everything else.
...
class ApiSchemaTestTable(Base):
    __abstract__ = True

    __table_args__: Any = {"schema": Schemas.API}


class OtherSchemaTestTable(Base):
    """A second schema, so the cross-schema relationships below get exercised."""

    __abstract__ = True

    __table_args__: Any = {"schema": Schemas.STAGING}


class BaseLookupTable(ApiSchemaTestTable, LookupTable):
    __abstract__ = True
...
class ExampleTable(ApiSchemaTestTable, TimestampMixin):
...
class FriendTable(OtherSchemaTestTable, TimestampMixin):
...
class LinkFriendType(OtherSchemaTestTable, TimestampMixin):
...
class SharedUser(BaseUser, OtherSchemaTestTable, TimestampMixin):
...
class SharedLinkExternalUser(BaseLinkExternalUser, OtherSchemaTestTable, TimestampMixin):
...
class SharedUserTokenSession(BaseUserTokenSession, OtherSchemaTestTable, TimestampMixin):
...
class SharedLoginGovState(BaseLoginGovState, OtherSchemaTestTable, TimestampMixin):
...
class SharedUserApiKey(BaseUserApiKey, OtherSchemaTestTable, TimestampMixin):
