...
def test_list_accessible_opportunities_success(client, db_session, enable_factory_create):
    ...
    inaccessible_agency = AgencyFactory.create()

    opp1 = OpportunityFactory.create(
        agency_id=agency1.agency_id,
        agency_code=agency1.agency_code,
        opportunity_number="ACCESSIBLE-2026-001",
    )
    opp2 = OpportunityFactory.create(
        agency_id=agency2.agency_id,
        agency_code=agency2.agency_code,
        opportunity_number="ACCESSIBLE-2026-002",
    )
    OpportunityFactory.create(
        agency_id=inaccessible_agency.agency_id,
        agency_code=inaccessible_agency.agency_code,
        opportunity_number="INACCESSIBLE-2026-001",
    )
...
def test_list_accessible_opportunities_with_pagination(client, db_session, enable_factory_create):
    ...
    for i in range(5):
        OpportunityFactory.create(
            agency_id=agency.agency_id,
            agency_code=agency.agency_code,
            opportunity_number=f"ACCESSIBLE-2026-{i:03d}",
        )
...
def test_list_accessible_opportunities_award_recommendation_ready_filter(
...
def test_list_accessible_opportunities_excludes_agencies_without_view_privilege(
    client, db_session, enable_factory_create
):
    user, agency_with_access, token, _ = create_user_in_agency_with_jwt_and_api_key(
        db_session=db_session,
        privileges=[Privilege.VIEW_OPPORTUNITY],
    )

    agency_without_access = AgencyFactory.create()
    give_user_privilege_in_agency(
        user=user,
        agency=agency_without_access,
        privileges=[],
    )

    accessible_opp = OpportunityFactory.create(
        agency_id=agency_with_access.agency_id,
        agency_code=agency_with_access.agency_code,
        opportunity_number="ACCESSIBLE-2026-001",
    )
    OpportunityFactory.create(
        agency_id=agency_without_access.agency_id,
        agency_code=agency_without_access.agency_code,
        opportunity_number="NO-VIEW-2026-001",
    )
    ...
    data = response.get_json()["data"]
    assert len(data) == 1
    assert data[0]["opportunity_id"] == str(accessible_opp.opportunity_id)
    assert data[0]["opportunity_number"] == "ACCESSIBLE-2026-001"


def test_list_accessible_opportunities_no_access_returns_empty(
    client, db_session, no_permission_auth_data
):
    user, agency, token, _ = no_permission_auth_data
...
def test_list_accessible_opportunities_invalid_token(client, db_session, grantor_auth_data):
    user, agency, token, _ = grantor_auth_data
...
def test_list_accessible_opportunities_empty_result(client, db_session, grantor_auth_data):
    user, agency, token, _ = grantor_auth_data
