...
def test_delete_attachment_success(client, db_session, grantor_auth_data, existing_opportunity, mock_opportunity_attachment):
    ...
    attachment_before = db_session.execute(
        select(OpportunityAttachment).where(
            OpportunityAttachment.attachment_id == attachment_id
        )
    ).scalar_one_or_none()
...
def test_delete_attachment_unauthorized(client, db_session, existing_opportunity, mock_opportunity_attachment):
...
def test_delete_attachment_published_opportunity(client, db_session, grantor_auth_data, monkeypatch):
    ...
    published_opportunity = OpportunityFactory.create(
        ...
        is_draft=False  # Published opportunity
    )
...
def test_delete_attachment_api_key_auth(client, grantor_auth_data, existing_opportunity, mock_opportunity_attachment):
    """Test that API key auth works for attachment deletion"""
    ...
    resp = client.delete(
        f"/v1/grantors/opportunities/{opportunity_id}/attachments/{attachment_id}",
        headers={"X-SGG-API-Key": api_key_id},
    )
