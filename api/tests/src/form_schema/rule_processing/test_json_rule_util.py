...
@pytest.mark.parametrize("existing_json,path,value,expected_json",[
    ...
    ({"other_name": "Joe", "nested": {"color": "Blue"}}, ["nested", "path", "to", "my_name"], "Steve", {"other_name": "Joe", "nested": {"color": "Blue", "path": {"to": {"my_name": "Steve"}}}}),
    ({"some_field": 5, "another_field": True}, ["nested", "field"], 10, {"some_field": 5, "another_field": True, "nested": {"field": 10}}),
...
def test_populate_nested_value_non_dict_in_path():
    with pytest.raises(ValueError, match="Unable to populate nested value, value in path is not a dictionary: my_field.nested"):
        populate_nested_value({"my_field": 10}, ["my_field", "nested"], "hello")


@pytest.mark.parametrize("json_data,path,expected_value",[
    ...
    ({"nested": {"path": {"to": {"value": "hello"}}}}, ["nested"], {"path": {"to": {"value": "hello"}}}),
    ({"nested": {"path": ["hello", "there", "this is a text"]}}, ["nested", "path"], ["hello", "there", "this is a text"]),
