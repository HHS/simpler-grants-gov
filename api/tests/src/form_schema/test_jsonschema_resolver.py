...
def test_resolver_nothing_to_resolve():
    ...
    schema = {
        ...
        "properties": {
            "field_a": {"type": "string", "title": "Field A", "description": "The field associated with A"},
            "field_b": {"type": "array", "minItems": 1, "items": {"type": "string"}},
            "field_c": {"type": "integer", "enum": [1, 2, 3]},
            "field_d": {"type": "object", "required": ["field_d_a"], "properties": {"field_d_a": {"type": "string"}}},
...
def test_resolve_jsonschema_defs():
    schema = {
        ...
        "$defs": {
            ...
            "my_def_b": {"type": "object", "properties": {
    ...
    expected_schema["$defs"]["my_def_b"]["properties"]["nested_a"] = expected_schema["$defs"]["my_def_a"]
    expected_schema["$defs"]["my_def_b"]["properties"]["nested_a_allof"]["allOf"][0] = expected_schema["$defs"]["my_def_a"]

    expected_schema["properties"]["field_a"] = expected_schema["$defs"]["my_def_a"]
    expected_schema["properties"]["field_a_allof"]["allOf"][0] = expected_schema["$defs"]["my_def_a"]
    ...
    expected_schema["properties"]["field_b_allof"]["allOf"][0] = expected_schema["$defs"]["my_def_b"]
...
def test_resolve_jsonschema_shared_schema():
    schema = {
        ...
        "properties": {
            ...
            "attachment": {"allOf": [{"$ref": COMMON_SHARED_V1.field_ref("attachment_field")}], "title": "Attachment", "description": "Attach it!"},
    ...
    resolved_address["properties"]["state"]["allOf"][0] = ADDRESS_SHARED_V1.json_schema["state_code"]
    resolved_address["properties"]["country"]["allOf"][0] = ADDRESS_SHARED_V1.json_schema["country_code"]
    expected_schema["properties"]["address"] = resolved_address

    resolved_simple_address = ADDRESS_SHARED_V1.json_schema["simple_address"]
    resolved_simple_address["properties"]["state"]["allOf"][0] = ADDRESS_SHARED_V1.json_schema["state_code"]
    expected_schema["properties"]["simple_address"]["allOf"][0] = resolved_simple_address

    expected_schema["properties"]["attachment"]["allOf"][0] = COMMON_SHARED_V1.json_schema["attachment_field"]
    expected_schema["properties"]["person_name"]["allOf"][0] = COMMON_SHARED_V1.json_schema["person_name"]
