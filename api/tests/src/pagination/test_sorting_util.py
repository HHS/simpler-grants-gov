...
def test_apply_sorting_with_mapping_nulls_last():
    ...
    validate_order_by(result, "ORDER BY api.example.example_id ASC NULLS LAST")
    ...
    validate_order_by(
        result,
        "ORDER BY api.example.description DESC NULLS LAST, api.example.my_count ASC NULLS LAST",
    )
    ...
    validate_order_by(
        result,
        "ORDER BY api.example.description ASC NULLS LAST, api.example.example_id DESC NULLS LAST, api.example.my_count DESC NULLS LAST",
    )
...
def test_apply_sorting_with_mapping_no_nulls_last():
    ...
    validate_order_by(
        result,
        "ORDER BY api.example.description DESC, api.example.my_count ASC",
    )
...
def test_apply_sorting_with_model_getattr():
    ...
    validate_order_by(
        result,
        "ORDER BY api.example.example_id ASC, api.example.my_count DESC",
    )
    assert "NULLS LAST" not in str(result.compile())
...
def test_apply_sorting_with_model_getattr_nulls_last():
    ...
    validate_order_by(result, "ORDER BY api.example.my_count ASC NULLS LAST")
