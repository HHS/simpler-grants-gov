...
from tests.src.db.models.factories import TransferTopportunityFactory, ForeignTopportunityFactory
import pytest
...
def truncate_foreign_tables(db_session):
...
def build_foreign_opportunity(db_session, opp_params: dict):
    ...
    db_session.execute(text(f"INSERT INTO foreign_topportunity ({','.join(columns)}) VALUES ({','.join(values)})"))
...
def test_copy_oracle_data(db_session, enable_factory_create):
    ...
    for copied_opportunity, foreign_record in zip(copied_opportunities, foreign_records, strict=True):
