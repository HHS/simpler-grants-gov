...
def test_get_soap_jwt_auth_jwt(enable_factory_create, caplog):
    ...
    mock_soap_config = get_soap_config()
    ...
    original_claims = jwt.decode(
        decoded_base64, key=mock_soap_config.soap_partner_gateway_auth_key, algorithms=["HS256"]
    )
    expected = {
        "sub": "partner_soap_call",
        "iss": f"{mock_soap_config.soap_partner_gateway_uri}",
        "exp": 1712145660,
