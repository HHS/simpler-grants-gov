...
class TestStoreOpportunityVersionTask(BaseTestClass):
    ...
    def test_store_opportunity_version_task(
        ...
        OpportunityChangeAuditFactory.create(opportunity=opportunities[0])
        ...
        OpportunityChangeAuditFactory.create(opportunity=opportunities[1])
        ...
        assert (
            len(
                [
                    opp
                    for opp in saved_opp_version
                    if opp.opportunity_id == opportunities[0].opportunity_id
                ]
            )
            == 2
        )  # updated twice
