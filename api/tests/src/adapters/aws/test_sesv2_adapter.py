...
def test_paginated_ses_adapter():
    ...
    # First Page
    resp = client.list_suppressed_destinations()
    assert len(resp.suppressed_destination_summaries) == 1
    assert (
        resp.suppressed_destination_summaries[0].email_address == "bounce@simulator.amazonses.com"
    )
    assert resp.next_token is not None
    next_token = resp.next_token

    # Second Page / Last Page
    resp_2 = client.list_suppressed_destinations(next_token=next_token)
    assert len(resp_2.suppressed_destination_summaries) == 1
    assert (
        resp_2.suppressed_destination_summaries[0].email_address
        == "bounce2@simulator.amazonses.com"
    )
    assert resp_2.next_token is None
