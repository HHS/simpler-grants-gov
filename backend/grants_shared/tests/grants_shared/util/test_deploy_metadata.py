...
from grants_shared.util.deploy_metadata import DeployMetadataConfig
...
def test_deploy_metadata_all_none(monkeypatch):
    monkeypatch.delenv("DEPLOY_GITHUB_REF", raising=False)
    monkeypatch.delenv("DEPLOY_GITHUB_SHA", raising=False)
