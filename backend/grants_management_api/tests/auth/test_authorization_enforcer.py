...
def test_user_with_no_roles_cannot_access_anything(
    ...
    assert (
        AuthorizationEnforcer(db_session).can_access(
            user, {MgmtPrivilege.VIEW_DEPARTMENT}, department_a
        )
        is False
    )
    assert (
        AuthorizationEnforcer(db_session).can_access(
            user, {MgmtPrivilege.VIEW_DEPARTMENT}, department_b
        )
        is False
    )
    assert (
        AuthorizationEnforcer(db_session).can_access(
            user, {MgmtPrivilege.VIEW_SUBAGENCY}, subagency_x
        )
        is False
    )
    assert (
        AuthorizationEnforcer(db_session).can_access(
            user, {MgmtPrivilege.VIEW_SUBAGENCY}, subagency_y
        )
        is False
    )
    assert (
        AuthorizationEnforcer(db_session).can_access(
            user, {MgmtPrivilege.VIEW_SUBAGENCY}, subagency_z
        )
        is False
    )
    assert (
        AuthorizationEnforcer(db_session).can_access(user, {MgmtPrivilege.VIEW_TEAM}, team1)
        is False
    )
    assert (
        AuthorizationEnforcer(db_session).can_access(user, {MgmtPrivilege.VIEW_TEAM}, team2)
        is False
    )
    assert (
        AuthorizationEnforcer(db_session).can_access(user, {MgmtPrivilege.VIEW_TEAM}, team3)
        is False
    )
    assert (
        AuthorizationEnforcer(db_session).can_access(user, {MgmtPrivilege.VIEW_TEAM}, team4)
        is False
    )
    assert (
        AuthorizationEnforcer(db_session).can_access(
            user, {MgmtPrivilege.VIEW_TEAM}, internal_resource1
        )
        is False
    )
    assert (
        AuthorizationEnforcer(db_session).can_access(
            user, {MgmtPrivilege.VIEW_TEAM}, internal_resource2
        )
        is False
    )


def test_user_department_level(
    db_session, department_a, department_b, subagency_x, team1, internal_resource1
):

    user = setup_user_with_roles(
        db_session, resources=[department_a], privileges=[MgmtPrivilege.VIEW_DEPARTMENT]
    )
    ...
    assert (
        AuthorizationEnforcer(db_session).can_access(
            user, {MgmtPrivilege.UPDATE_DEPARTMENT}, department_a
        )
        is False
    )
    ...
    assert (
        AuthorizationEnforcer(db_session).can_access(
            user, {MgmtPrivilege.VIEW_DEPARTMENT, MgmtPrivilege.UPDATE_DEPARTMENT}, department_a
        )
        is False
    )
    ...
    assert (
        AuthorizationEnforcer(db_session).can_access(
            user, {MgmtPrivilege.VIEW_DEPARTMENT}, subagency_x
        )
        is True
    )
    assert (
        AuthorizationEnforcer(db_session).can_access(user, {MgmtPrivilege.VIEW_DEPARTMENT}, team1)
        is True
    )
    # No hierarchy gets to internal resources
    assert (
        AuthorizationEnforcer(db_session).can_access(
            user, {MgmtPrivilege.VIEW_DEPARTMENT}, internal_resource1
        )
        is False
    )


def test_user_subagency_level(
    db_session, department_a, subagency_x, subagency_y, team1, internal_resource1
):
    user = setup_user_with_roles(
        db_session, resources=[subagency_x], privileges=[MgmtPrivilege.VIEW_SUBAGENCY]
    )
    ...
    assert (
        AuthorizationEnforcer(db_session).can_access(
            user, {MgmtPrivilege.UPDATE_SUBAGENCY}, subagency_x
        )
        is False
    )
    # User only has part of these privileges, so is denied
    assert (
        AuthorizationEnforcer(db_session).can_access(
            user, {MgmtPrivilege.VIEW_SUBAGENCY, MgmtPrivilege.UPDATE_SUBAGENCY}, subagency_x
        )
        is False
    )
    ...
    assert (
        AuthorizationEnforcer(db_session).can_access(
            user, {MgmtPrivilege.VIEW_SUBAGENCY}, department_a
        )
        is False
    )
    # User could view subagency against the team under the subagency
    assert (
        AuthorizationEnforcer(db_session).can_access(user, {MgmtPrivilege.VIEW_SUBAGENCY}, team1)
        is True
    )
    ...
    assert (
        AuthorizationEnforcer(db_session).can_access(
            user, {MgmtPrivilege.VIEW_SUBAGENCY}, internal_resource1
        )
        is False
    )
...
def test_user_team_level(db_session, department_a, subagency_x, team1, team2, internal_resource1):
    user = setup_user_with_roles(
        db_session, resources=[team1], privileges=[MgmtPrivilege.VIEW_TEAM]
    )

    # User can view their team
    assert (
        AuthorizationEnforcer(db_session).can_access(user, {MgmtPrivilege.VIEW_TEAM}, team1) is True
    )
    # User does not have access to edit the team
    assert (
        AuthorizationEnforcer(db_session).can_access(user, {MgmtPrivilege.UPDATE_TEAM}, team1)
        is False
    )
    # User only has part of these privileges, so is denied
    assert (
        AuthorizationEnforcer(db_session).can_access(
            user, {MgmtPrivilege.VIEW_TEAM, MgmtPrivilege.UPDATE_TEAM}, team1
        )
        is False
    )
    ...
    assert (
        AuthorizationEnforcer(db_session).can_access(user, {MgmtPrivilege.VIEW_TEAM}, department_a)
        is False
    )
    # User cannot view team against the parent subagency
    assert (
        AuthorizationEnforcer(db_session).can_access(user, {MgmtPrivilege.VIEW_TEAM}, subagency_x)
        is False
    )
...
def test_user_internal_resource(
    db_session, internal_resource1, internal_resource2, department_a, subagency_x, team1
):
    user = setup_user_with_roles(
        db_session, resources=[internal_resource1], privileges=[MgmtPrivilege.VIEW_TEAM]
    )
    ...
    assert (
        AuthorizationEnforcer(db_session).can_access(
            user, {MgmtPrivilege.UPDATE_TEAM}, internal_resource1
        )
        is False
    )
    # User only has part of these privileges, so is denied
    assert (
        AuthorizationEnforcer(db_session).can_access(
            user, {MgmtPrivilege.VIEW_TEAM, MgmtPrivilege.UPDATE_TEAM}, internal_resource1
        )
        is False
    )
    ...
    for resource in [department_a, subagency_x, team1]:
        assert (
            AuthorizationEnforcer(db_session).can_access(user, {MgmtPrivilege.VIEW_TEAM}, resource)
            is False
        )
...
def test_user_with_multiple_privileges_in_role(db_session, internal_resource1, internal_resource2):
    ...
    assert (
        AuthorizationEnforcer(db_session).can_access(
            user, {MgmtPrivilege.VIEW_TEAM, MgmtPrivilege.CREATE_TEAM}, internal_resource1
        )
        is False
    )
    assert (
        AuthorizationEnforcer(db_session).can_access(
            user,
            {MgmtPrivilege.VIEW_TEAM, MgmtPrivilege.UPDATE_TEAM, MgmtPrivilege.DELETE_TEAM},
            internal_resource1,
        )
        is False
    )
    assert (
        AuthorizationEnforcer(db_session).can_access(
            user, {MgmtPrivilege.MANAGE_TEAM_MEMBERS}, internal_resource1
        )
        is False
    )
    ...
    assert (
        AuthorizationEnforcer(db_session).can_access(
            user, {MgmtPrivilege.UPDATE_TEAM}, internal_resource2
        )
        is False
    )
    assert (
        AuthorizationEnforcer(db_session).can_access(
            user, {MgmtPrivilege.VIEW_TEAM, MgmtPrivilege.UPDATE_TEAM}, internal_resource2
        )
        is False
    )
...
def test_user_with_privileges_across_roles(db_session, internal_resource1, internal_resource2):
    ...
    role1 = MgmtRoleFactory.create(
        privileges=[MgmtPrivilege.VIEW_TEAM], resource_types=[MgmtResourceType.INTERNAL]
    )
    role2 = MgmtRoleFactory.create(
        privileges=[MgmtPrivilege.UPDATE_TEAM], resource_types=[MgmtResourceType.INTERNAL]
    )
...
def test_user_with_multiple_roles_at_different_levels(
    db_session, department_a, subagency_x, team1, team2, team3, team4
):

    # Has 3 different team related privileges at department/subagency/team levels
    department_role = MgmtRoleFactory.create(
        privileges=[MgmtPrivilege.VIEW_TEAM], resource_types=[MgmtResourceType.DEPARTMENT]
    )
    subagency_role = MgmtRoleFactory.create(
        privileges=[MgmtPrivilege.UPDATE_TEAM], resource_types=[MgmtResourceType.SUBAGENCY]
    )
    team_role = MgmtRoleFactory.create(
        privileges=[MgmtPrivilege.MANAGE_TEAM_MEMBERS], resource_types=[MgmtResourceType.TEAM]
    )
    ...
    assert (
        AuthorizationEnforcer(db_session).can_access(
            user, {MgmtPrivilege.MANAGE_TEAM_MEMBERS}, team1
        )
        is True
    )
    ...
    assert (
        AuthorizationEnforcer(db_session).can_access(
            user,
            {MgmtPrivilege.VIEW_TEAM, MgmtPrivilege.UPDATE_TEAM, MgmtPrivilege.MANAGE_TEAM_MEMBERS},
            team1,
        )
        is True
    )
    ...
    assert (
        AuthorizationEnforcer(db_session).can_access(user, {MgmtPrivilege.DELETE_TEAM}, team1)
        is False
    )
    assert (
        AuthorizationEnforcer(db_session).can_access(user, {MgmtPrivilege.CREATE_TEAM}, team1)
        is False
    )

    # Can do the actions from the department/subagency against a sibling team
    assert (
        AuthorizationEnforcer(db_session).can_access(user, {MgmtPrivilege.VIEW_TEAM}, team2) is True
    )
    assert (
        AuthorizationEnforcer(db_session).can_access(user, {MgmtPrivilege.UPDATE_TEAM}, team2)
        is True
    )
    assert (
        AuthorizationEnforcer(db_session).can_access(
            user, {MgmtPrivilege.MANAGE_TEAM_MEMBERS}, team2
        )
        is False
    )
    ...
    assert (
        AuthorizationEnforcer(db_session).can_access(user, {MgmtPrivilege.VIEW_TEAM}, team3) is True
    )
    assert (
        AuthorizationEnforcer(db_session).can_access(user, {MgmtPrivilege.UPDATE_TEAM}, team3)
        is False
    )
    assert (
        AuthorizationEnforcer(db_session).can_access(
            user, {MgmtPrivilege.MANAGE_TEAM_MEMBERS}, team3
        )
        is False
    )
    ...
    assert (
        AuthorizationEnforcer(db_session).can_access(user, {MgmtPrivilege.UPDATE_TEAM}, team4)
        is False
    )
    assert (
        AuthorizationEnforcer(db_session).can_access(
            user, {MgmtPrivilege.MANAGE_TEAM_MEMBERS}, team4
        )
        is False
    )
...
def test_logging(db_session, department_a, team1, subagency_x):
    ...
    assert set(log_context["required_privileges"].split("|")) == {
    ...
    assert set(log_context["authorizing_role_ids"].split("|")) == {
        str(department_role.mgmt_role_id),
        str(subagency_role.mgmt_role_id),
        str(team_role.mgmt_role_id),
    }
    assert set(log_context["authorizing_role_names"].split("|")) == {
        department_role.role_name,
        subagency_role.role_name,
        team_role.role_name,
    }
