...
Environment variables are defined in the `app-config` module in the [environment_variables.tf file](../../infra/api/app-config/env-config/environment_variables.tf). Modify the `default_extra_environment_variables` map to define extra environment variables specific to the application. Map keys define the environment variable name, and values define the default value for the variable across application environments. For example:
